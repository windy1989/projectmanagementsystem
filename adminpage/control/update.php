<?php 
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if($metode == "dbupload"){
			...
			// MySQL username
			$mysql_username = 'cvvictor_adm';
			// MySQL password
			$mysql_password = 'Ird256o+9=$l';
			// Database name
			$mysql_database = 'cvvictor_app';
			...
			echo "1";
			$con->close();
		}elseif($metode == "download"){
			if(file_exists("../backup/db.sql")){
			 ...
			 */
			define("DB_USER", 'cvvictor_adm');
			define("DB_PASSWORD", 'Ird256o+9=$l');
			define("DB_NAME", 'cvvictor_app');
			...
			?>
			<script>
				window.open('adminpage/backup/db.sql', '_self');
		...
		$stmt = $dbh->prepare("SELECT * FROM sim_update ORDER BY ID DESC");
		$stmt->execute();
		require 'adminpage/pages/simupdate.php';
	}
?>